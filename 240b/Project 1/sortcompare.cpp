...
#include "mergesort.cpp"        //merge_sort_pub
#include "simplesort.cpp"       //simple_sort
#include "itermergesort.cpp"    //iter_merge_sort
...
double run_sort(void (*sortfunc)(int*, int), int test_data[], int test_size, int seed);
...
const int num_sorts = 4;

int main()
{
    //Declare an array to hold pointers to all of the sort functions.
    //Declare an array of strings to hold the name of each sort.
    //Declare an array of booleans to keep track of whether or not each sort should be performed.
    void (*sorts[num_sorts])(int*, int) = {&simple_sort, &merge_sort_pub, &iter_merge_sort, &stl_sort};
    char *sort_names[num_sorts] = {"SimpleSort", "MergeSort-R", "MergeSort-I", "STL Sort"};
    ...
    int *test_data = new int[max_size];
    ...
    for(int test_size = step; test_size <= max_size; test_size += step) {
        ...
        for(int x = 0; x < num_sorts; ++x) {
            if(sort_switches[x]) {
                time_taken = run_sort(sorts[x], test_data, test_size, seed);
                output << "," << time_taken;
            }
        }
...
double run_sort(void (*sortfunc)(int*, int), int test_data[], int test_size, int seed)
{
    ...
    (*sortfunc)(test_data, test_size);//fix for STL sort
