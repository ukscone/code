...
#include "connectionexception.h" //Provides ConnectionException
using namespace std;
...
game::who game::play() { 
	...
	while (!is_game_over())
	{
	    ...
	    if (next_player() == HUMAN) 
	    	make_human_move();
	    else if(next_player() == COMPUTER)
			make_computer_move();
		else if(next_player() == NET) {
			//Check for a lost connection.
			try	{make_net_move();}
			catch(ConnectionException e) {
				display_message("Connection lost. Press any key to exit.");
				cin.get();
				return NEUTRAL;
			}
		}
...
// moved.
int game::eval_with_lookahead(int look_ahead, int beat_this) { //Minimax
    ...
    // Base case:
	if (look_ahead == 0 || is_game_over()) //Heuristic value of node
	{
		if (last_mover() == PLAYER2) 
		    return evaluate();
		else
			return -evaluate();
	}
...
void game::make_computer_move() {
	...
	compute_moves(moves);
	//assert(!moves.empty()); 
	//In Reversi, the game only ends when *both* players have no moves left.
	if(moves.empty()) {		 
		best_move = "pass";
	}
	...
	// Make the best move.
	make_move(best_move);
	if(is_net_game) send_net_move(best_move); 
}
...
void game::make_human_move() {
	...
	make_move(move);
	if(is_net_game) send_net_move(move); 
}

void game::make_net_move() {
	string move;
	
	display_message("Waiting for opponent's move...");
	
	int status = get_net_move(move);
	
	//If the move we recieved isn't legal, 
	//there's something wrong with our connection.
	if(!(is_legal(move) && status)) { 
...
void game::set_player_types(PLAYER player_one, PLAYER player_two) {
 		if(player_one == NET || player_two == NET) is_net_game = true;
 		else is_net_game = false;
 		player1 = player_one;
 		player2 = player_two;
}
